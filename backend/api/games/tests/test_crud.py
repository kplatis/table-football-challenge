...
class TestGameCrud:
    ...
    def test_create_game_success(self, test_main_database):
        """
    ...
    def test_create_game_without_goals_provided_have_0_goals(self, test_main_database):
        """
        Tests the CRUD action to create a new game
        """
        game_data = GameCreate(first_team_id=1, second_team_id=2)
    ...
    def test_game_creation_throws_exception_if_same_team_id(self, test_main_database):
        """
    ...
    def test_games_listing_returns_all_games(self, test_main_database):
        """
    ...
    def test_game_patching_successfully_updates_the_game(self, test_main_database):
        """
        Tests whether a game is correctly updated
        """

        game_data = GamePartialUpdate(first_team_goals=2, second_team_goals=1)
        updated_game = partial_update_game(
            db=test_main_database, game_id=1, game=game_data
        )
        assert updated_game.first_team_goals == 2
        assert updated_game.second_team_goals == 1

    def test_game_patching_raises_exception_if_game_does_not_exist(
        self, test_main_database
    ):
        """
    ...
    def test_single_game_retrieval_returns_correct_game(self, test_main_database):
        """
    ...
    def test_single_game_retrieval_raises_exception_if_not_exist(
        self, test_main_database
    ):
        """
