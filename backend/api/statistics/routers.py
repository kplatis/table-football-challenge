...
async def get_statistics_overview_for_teams_and_players(db: Session = Depends(get_db)):
    """
    ...
    team_response = list(team_ids_to_statistics.values()) + player_statistics
    return [StatisticsByTeamOrPlayer(**response) for response in team_response]
